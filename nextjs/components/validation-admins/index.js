    ...
    */
    ...
    return (
        <div className='validation-admins'>
            {!hasUser && !openValidation &&
                <button className='bg-yellow-400 cursor-pointer h-60 w-full  flex flex-col rounded-xl justify-center px-4 text-left'
                    onClick={() => { setOpenValidation(true) }}>
                    <span className='py-4 font-semibold'>Inscriptions des utilisateurs</span>
                    <span className='py-4'>Vous retrouvrez l'ensemble des consultants et recruteurs nouvellement inscrits qui attendent votre validation</span>
                </button>
            }

            {openValidation &&
                <div>
                    <button onClick={() => setOpenValidation(false)}>close</button>
